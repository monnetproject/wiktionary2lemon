...
/**
 * Bag-of-words cosine similarity after punctuation has been removed.
 * Intralingual aggregation: maximum
...
public class AverageMaximumBagOfWordsCosineNoPunct implements EntitySimilarityMeasure {
	...
	public AverageMaximumBagOfWordsCosineNoPunct() {
	}
	...
	public double getScore(Entity srcEntity, Entity tgtEntity) {
		...
		for (Language language : this.languages) {
			...
			for (String srcLabel : srcLabels) {
				...
				List<Token> srcTokens = tokenizer.tokenize(SimilarityUtils.removePunctuation(srcLabel));
				
				for (String tgtLabel : tgtLabels) {
					
					List<Token> tgtTokens = tokenizer.tokenize(SimilarityUtils.removePunctuation(tgtLabel));
			...
			intralingualScores.add(Functions.max(scores));
