...
/**
 * Similarity measure based on difference between minimum number of direct children in the calculations (for monetary items) or presentations (for string items) of the entities.
 * 
 * @author Dennis Spohr
 *
 */
@Component(provide=EntitySimilarityMeasure.class)
public class MinimumDirectChildren implements EntitySimilarityMeasure {
	
    private Logger log = Logging.getLogger(this);
	private final String name = this.getClass().getName();
	
	public MinimumDirectChildren() {
	}
	...
	public double getScore(Entity srcEntity, Entity tgtEntity) {
		...
		double srcAvg = Functions.min(srcItems);
		
		double tgtAvg = Functions.min(tgtItems);
